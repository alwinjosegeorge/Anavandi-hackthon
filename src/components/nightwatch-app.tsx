...
function DesktopNavBar({
  ...
  return (
    <header className="hidden lg:flex sticky top-0 z-50 h-16 w-full items-center justify-between border-b bg-surface/95 px-8 backdrop-blur shadow-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          <span className="text-[11px]">Systems Ready</span>
        </button>

        <Button
          variant="destructive"
          size="sm"
          className="font-extrabold text-xs shadow-soft"
...
export function NightwatchApp() {
  ...
  const content = (() => {
    ...
    if (view === "home") return (
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 pb-28 lg:pb-12">
        <div className="lg:grid lg:grid-cols-[460px_1fr] lg:gap-8 lg:items-start">
          {/* Left Column: Journey Controls & Status (Exact Phone UI on mobile) */}
          ...
          {/* Right Column (Desktop website only): Kerala Live Route Map & Dashboard Metrics */}
    ...
    if (view === "plan") return (
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5 pb-28 lg:pb-12">
        ...
        <div className="lg:grid lg:grid-cols-[460px_1fr] lg:gap-8 lg:items-start">
          ...
          {/* Right Column (Desktop only): Full Map View with Checkpoints */}
  ...
  return (
    <div className={cn("nightwatch-root", dark && "dark")}>
      ...
      {framed && (
        <div className="fixed right-3 top-3 z-[80] hidden items-center gap-2 lg:flex">
          <Button variant="outline" size="sm" onClick={() => setFramed(false)}>
            <Smartphone className="size-3.5 mr-1" /> Web View
          </Button>
        </div>
      )}
      <div className={cn("app-stage", framed && view !== "contact" ? "device-stage" : "web-stage", view === "contact" && "contact-stage")}>
        <div className={cn("app-viewport", framed && view !== "contact" ? "device-frame" : "web-viewport")}>
          {framed && view !== "contact" && <div className="device-island" />}
          <div className={cn("app-scroll-body flex-1", showNav && !framed && "pb-24 lg:pb-8")}>
            {content}
          </div>
          {showNav && (
            <nav
              className={cn(
                ...
                framed && view !== "contact"
                  ...
                  : "fixed bottom-0 inset-x-0 mx-auto max-w-lg w-full lg:hidden"
              )}
            >
