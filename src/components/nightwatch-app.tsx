...
function MapArtwork({ deviation = false, compact = false }: { deviation?: boolean; compact?: boolean }) {
  return (
    <div className={cn("relative overflow-hidden bg-map", compact ? "h-56" : "h-full min-h-96")}>
      <svg viewBox="0 0 420 600" className="absolute inset-0 h-full w-full" aria-label="Journey route map">
        <path d="M-10 85 C90 112 115 36 236 82 S365 151 440 108" className="map-road-wide" />
        <path d="M54 -10 C61 87 140 139 126 243 S72 390 119 620" className="map-road" />
        <path d="M305 -20 C276 87 337 156 304 245 S228 419 337 620" className="map-road" />
        <path d="M-30 322 C90 260 159 338 260 307 S371 270 450 332" className="map-road" />
        <path d="M-20 486 C92 435 166 505 250 464 S356 419 452 443" className="map-road-thin" />
        <path d="M34 178 C102 182 162 137 234 164 S352 237 430 210" className="map-road-thin" />
        {deviation && <path d="M78 514 C124 432 176 393 246 328 S310 234 330 128" className="map-route-planned" />}
        <path d={deviation ? "M78 514 C126 433 177 418 151 346 S213 251 330 128" : "M78 514 C124 432 176 393 246 328 S310 234 330 128"} className="map-route" />
        {["78,514", "170,401", "246,328", "290,226", "330,128"].map((point, index) => {
          const [cx, cy] = point.split(",");
          return <circle key={point} cx={cx} cy={cy} r={index === 0 || index === 4 ? 11 : 6} className={index === 0 || index === 4 ? "map-marker" : "map-checkpoint"} />;
        })}
        <circle cx={deviation ? 151 : 226} cy={deviation ? 346 : 350} r="21" className="map-user-ring" />
        <circle cx={deviation ? 151 : 226} cy={deviation ? 346 : 350} r="8" className="map-user" />
      </svg>
      <div className="absolute left-4 top-4 rounded-md bg-surface/90 px-3 py-2 text-xs font-semibold shadow-soft backdrop-blur">Kochi · Kakkanad</div>
      <div className="absolute bottom-4 right-4 grid size-10 place-items-center rounded-full bg-surface shadow-soft"><LocateFixed className="size-4 text-primary" /></div>
    </div>
  );
}
...
export function NightwatchApp() {
  ...
  const content = (() => {
    ...
    if (view === "details") return <><Header title="Journey details" back={() => go("history")}/><div className="px-5 pb-10"><div className="overflow-hidden rounded-lg border"><MapArtwork compact/></div><div className="my-4 grid grid-cols-3 rounded-lg border bg-surface p-4 text-center"><Metric value="14.2 km" label="Distance"/><Metric value="42 min" label="Duration"/><Metric value="Safe" label="Outcome"/></div><div className="rounded-lg border bg-surface p-5"><h2 className="mb-5 font-extrabold">Full timeline</h2><Timeline detailed/></div></div></>;
