...
export const KOCHI_LOCATIONS: LocationPoint[] = [
  { name: "Marine Drive", detail: "Ernakulam City Centre", coords: [9.9790, 76.2759] },
  { name: "MG Road Metro", detail: "Maharajas College / Shenoys", coords: [9.9714, 76.2842] },
  { name: "Kaloor Junction", detail: "Bus Stand & Metro Station", coords: [9.9988, 76.2927] },
  { name: "Palarivattom", detail: "Bypass Junction & Metro", coords: [10.0076, 76.3073] },
  { name: "Kakkanad InfoPark", detail: "SmartCity & Infopark Phase 1/2", coords: [10.0159, 76.3419] },
  { name: "Edappally Lulu", detail: "Lulu Mall & Toll Junction", coords: [10.0270, 76.3080] },
  { name: "Vyttila Mobility Hub", detail: "KSRTC & Metro & Water Metro", coords: [9.9698, 76.3190] },
  { name: "Fort Kochi", detail: "Kochi Beach & Vasco da Gama Square", coords: [9.9658, 76.2425] },
  { name: "Thykoodam", detail: "South Kochi Metro", coords: [9.9575, 76.3150] },
  { name: "Aluva Metro Station", detail: "Railway Station & Periyar", coords: [10.1076, 76.3516] },
  { name: "Cochin Int'l Airport (COK)", detail: "Nedumbassery", coords: [10.1518, 76.3929] },
];
...
function calculateTripStats(from: LocationPoint, to: LocationPoint) {
  ...
  const distanceKm = Math.max(1.2, Math.round(crowdDistanceKm * 1.35 * 10) / 10);
  const durationMins = Math.max(8, Math.round(distanceKm * 2.5 + 4));
...
export function NightwatchApp() {
  ...
  // Dynamic Departure and Destination states
  const [fromLoc, setFromLoc] = useState<LocationPoint>(KOCHI_LOCATIONS[0]); // Marine Drive
  const [toLoc, setToLoc] = useState<LocationPoint>(KOCHI_LOCATIONS[4]); // Kakkanad InfoPark
  ...
  const content = (() => {
        ...
        <div className="px-5 pb-28 pt-5">
          <div className="flex items-center justify-between">
            <div>
              ...
              <h1 className="text-2xl font-black">Alwin</h1>
            </div>
          ...
          <section className="mt-7 overflow-hidden rounded-lg bg-foreground p-5 text-primary-foreground shadow-lift">
            <div className="flex justify-between items-start">
              <div>
                <p className="text-xs font-bold uppercase text-primary">Night journey</p>
                <h2 className="mt-2 text-2xl font-black">Ready for a journey?</h2>
              </div>
              <Shield className="size-9 text-primary shrink-0" />
            </div>

            <div className="mt-6 rounded-md bg-primary-foreground/10 p-3 space-y-2">
              <div className="flex items-center justify-between gap-2">
                <button
                  className="flex flex-1 items-center gap-3 text-left p-2 rounded hover:bg-primary-foreground/10 transition-colors"
              ...
              <button
                className="flex w-full items-center gap-3 text-left p-2 rounded hover:bg-primary-foreground/10 transition-colors"
            ...
            <div className="mt-3 flex items-center justify-between text-xs text-primary-foreground/70 px-1">
          ...
          <section className="mt-5 grid grid-cols-2 gap-3">
            <button className="rounded-lg border bg-surface p-4 text-left shadow-soft" onClick={() => go("safety")}>
              <div className="flex items-center gap-2"><StatusDot /><span className="text-xs font-bold">ALL SYSTEMS READY</span></div>
              <p className="mt-5 text-sm font-bold">Safety status</p>
              <p className="text-xs text-muted-foreground">Everything looks good</p>
            </button>
            <button className="rounded-lg border bg-surface p-4 text-left shadow-soft" onClick={() => go("contacts")}>
          ...
          <section className="mt-7">
            <div className="flex items-center justify-between">
              <h2 className="font-extrabold">Recent journeys</h2>
              <Button variant="ghost" size="sm" onClick={() => go("history")}>View all</Button>
            </div>
            <button className="mt-3 flex w-full items-center gap-3 rounded-lg border bg-surface p-4 text-left">
              <div className="grid size-10 place-items-center rounded-md bg-primary-soft">
    ...
    if (view === "plan") return (
      <>
        ...
        <div className="space-y-5 px-5 pb-10">
          <div className="rounded-lg border bg-surface p-4 shadow-soft">
            <div className="flex items-center justify-between">
            ...
            <button
              onClick={() => { setPickerMode("from"); setSearchQuery(""); }}
              className="mt-2 flex w-full items-center gap-3 rounded-md bg-muted p-3 text-left hover:bg-muted/80 transition-colors"
            >
              <LocateFixed className="size-4 text-success shrink-0" />
              <div className="min-w-0 flex-1">
          ...
          <div className="overflow-hidden rounded-lg border bg-surface shadow-soft">
          ...
          <div className="rounded-lg border border-primary/30 bg-primary-soft p-4">
            <p className="text-xs font-black uppercase text-primary">Kochi Monitored Transit Corridor</p>
    ...
    if (view === "details") return <><Header title="Journey details" back={() => go("history")}/><div className="px-5 pb-10"><div className="overflow-hidden rounded-lg border"><NightwatchLeafletMap compact dark={dark} from={fromLoc} to={toLoc}/></div><div className="my-4 grid grid-cols-3 rounded-lg border bg-surface p-4 text-center"><Metric value={tripStats.distanceKm} label="Distance"/><Metric value={tripStats.durationMins} label="Duration"/><Metric value="Safe" label="Outcome"/></div><div className="rounded-lg border bg-surface p-5"><h2 className="mb-5 font-extrabold">Full timeline</h2><Timeline detailed/></div></div></>;
        ...
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-overlay p-0 sm:p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="flex max-h-[85vh] w-full max-w-lg flex-col rounded-t-2xl sm:rounded-2xl bg-surface p-5 shadow-lift overflow-hidden">
            ...
            <div className="relative mt-3">
              ...
              <input
                type="text"
                placeholder="Search transit hub, metro, airport..."
                value={searchQuery}
            ...
            {/* Locations list */}
            <div className="mt-3 flex-1 overflow-y-auto space-y-1 max-h-[300px] pr-1">
              <p className="text-[11px] font-bold uppercase text-muted-foreground px-1 py-1">
                Popular Kochi Transit Hubs
              </p>
              {KOCHI_LOCATIONS.filter((loc) =>
                ...
                return (
                  ...
                  >
                    <MapPin className={cn("size-4 shrink-0", isSelected ? "text-primary-foreground" : "text-primary")} />
                    <div className="min-w-0 flex-1">
                      <p className="truncate text-sm font-bold">{loc.name}</p>
                      {loc.detail && (
                        <p className={cn("truncate text-xs", isSelected ? "text-primary-foreground/80" : "text-muted-foreground")}>
                          {loc.detail}
                        </p>
                      )}
                    </div>
